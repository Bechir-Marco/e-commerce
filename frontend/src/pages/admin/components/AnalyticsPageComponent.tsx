...
type Props = {
    fetchOrdersForFirstDate: ( firstDateToCompare:any) => Promise<any>;
    fetchOrdersForSecondDate: (secondDateToCompare:any) => Promise<any>;
    socketIOClient: any;
    };
...
const AnalyticsPageComponent = ({
  ...
  const [dataForFirstSet, setDataForFirstSet] = useState<any>([]);
  const [dataForSecondSet, setDataForSecondSet] = useState<any>([]);

  useEffect(() => {
    const socket = socketIOClient();
    let today = new Date().toDateString();
    const handler = (newOrder:any) => {
  ...
  const firstDateHandler = (e:any) => {
    setFirstDateToCompare(e.target.value);
  };

  const secondDateHandler = (e:any) => {
