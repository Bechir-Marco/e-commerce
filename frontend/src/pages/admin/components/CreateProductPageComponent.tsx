...
const CreateProductPageComponent = ({
  ...
  deleteCategory,
  saveAttributeToCatDoc,
}:CreateProductPageComponentProps) => {
  ...
  const newCategoryHandler = (e:any) => {
    if (e.keyCode && e.keyCode === 13 && e.target.value) {
      ...
      setTimeout(() => {
          let element = document.getElementById('cats')as HTMLInputElement;
           if (element) { setCategoryChoosen(e.target.value);
  ...
  const deleteAttribute = (key:any) => {
    setAttributesTable((table) => table.filter((item:any) => item.key !== key));
  };
          ...
          >
            ...
            <Form.Group controlId="formFileMultiple" className="mb-3 mt-3">
              ...
              <Form.Control
                ...
                type="file"
                multiple
                onChange={(e:any) => uploadHandler(e.target.files)}
              />
              {isCreating}
            </Form.Group>
