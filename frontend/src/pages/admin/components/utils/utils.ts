export const changeCategory = (e:any, categories:any, setAttributesFromDb:any, setCategoryChoosen:any) => {
    const highLevelCategory = e.target.value.split("/")[0];
    const highLevelCategoryAllData = categories.find(
        (cat:any) => cat.name === highLevelCategory
    );
...
export const setValuesForAttrFromDbSelectForm = (e:any, attrVal:any, attributesFromDb:any) => {
    if (e.target.value !== "Choose attribute") {
        ...
        if (selectedAttr && selectedAttr.value.length > 0) {
            ...
            selectedAttr.value.map((item:any) => {
...
export const setAttributesTableWrapper = (key:any, val:any, setAttributesTable:any) => {
    setAttributesTable((attr:any) => {
        if (attr.length !== 0) {
            var keyExistsInOldTable = false;
            let modifiedTable = attr.map((item:any) => {
                if (item.key === key) {
